...
|---|---|---|---|---|
| R1 | Kill-ratio tuning still genuinely hard | 2 | Real iteration before the hackathon; curated fixtures as fallback | Open, first operator built: `app.domain.mutation` implements comparison-operator replacement (Eq/NotEq/Lt/GtE/Gt/LtE/In/NotIn/Is/IsNot), generic across any source, verified against a second, structurally different challenge (binary search vs. graph traversal) — 3/3 generated mutants killed by the existing property, confirming the operator and kernel genuinely generalize, not just work by coincidence for one case (`memory/episodic/0023-*.md`). Only one operator family exists; kill-ratio *tuning* and equivalent-mutant tolerance are still unaddressed |
