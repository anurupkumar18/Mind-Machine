...
|---|---|---|---|---|
| R1 | Kill-ratio tuning still genuinely hard | 2 | Real iteration before the hackathon; curated fixtures as fallback | Open, real progress: `app.domain.kill_ratio.classify_mutant` gives a correct killed/survived classification (a mutant that errors or times out is killed, not "surviving" — the ad hoc checks used before this undercounted kills). Reclassifying binary_search's 15 mutants (2 operator families) with it: 12/15 killed. Of the 3 survivors, one is a **confirmed genuine equivalent mutant** (`// 2` -> `// 1` degenerates into an O(n) scan with 0/2000 mismatches against the reference across randomized trials — not fixable by adding tests) and two were real test-input-set gaps, now closed by two added hidden test cases (one exposed a skipped-index bug, one exposed a genuine infinite-loop risk, safely caught by the sandbox's existing timeout as `killed` under the corrected classifier). See `memory/episodic/0025-*.md` |
