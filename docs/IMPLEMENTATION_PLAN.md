...
|---|---|---|---|---|
| R1 | Kill-ratio tuning still genuinely hard | 2 | Real iteration before the hackathon; curated fixtures as fallback | Open, but the pipeline named in this plan is now real end-to-end: `app.domain.mutation` generates candidates → `app.domain.kill_ratio.classify_mutant` classifies them → `app.domain.equivalence.check_equivalence` automatically distinguishes genuine equivalence from test-coverage gaps for survivors → `app.domain.content_selection.select_mutant` decides accept/reject/flag-for-review. Run against binary_search's real 15-mutant set: 14 accepted, 1 correctly rejected as the confirmed equivalent mutant, 0 needing human review (both real gaps from `episodic/0025` were already closed). Still open: only one challenge has been run through this whole pipeline in practice; the policy's flagged-for-review branch has no real example yet, only a unit test; still exactly two mutation-operator families. See `memory/episodic/0027-*.md` |
