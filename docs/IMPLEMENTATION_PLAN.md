...
|---|---|---|---|---|
| R1 | Kill-ratio tuning still genuinely hard | 2 | Real iteration before the hackathon; curated fixtures as fallback | Open, substantial progress: `app.domain.kill_ratio.classify_mutant` gives a correct killed/survived classification; `app.domain.equivalence.check_equivalence` now **automates** equivalent-mutant detection (differential testing against the oracle across N randomly generated valid inputs, reusing the real sandbox in one batched subprocess call rather than one-per-trial) — validated against the one confirmed genuine equivalent mutant (`// 2` -> `// 1`, still equivalent at 2000 trials) and a confirmed real bug (correctly flagged not-equivalent). Manual investigation (`episodic/0025`) is no longer required per mutant. Kill-ratio *filtering policy* (what to do with the classification, e.g. discard likely-equivalent mutants from a content catalog) is still unbuilt — the signal now exists, the policy on top of it doesn't. See `memory/episodic/0026-*.md` |
