...
|---|---|---|---|---|
| R1 | Kill-ratio tuning still genuinely hard | 2 | Real iteration before the hackathon; curated fixtures as fallback | Open. Two operator families now exist in `app.domain.mutation`: comparison-operator replacement (3/3 killed on binary_search) and integer-constant boundary mutation (+1/-1 on numeric literals) — the second family gives the first real *mixed* result, 7/12 killed on binary_search, including some mutants that error out rather than complete, real signal for future kill-ratio/equivalent-mutant work rather than the trivial 100%-kill dataset before it (`memory/episodic/0024-*.md`). Kill-ratio *tuning* and equivalent-mutant tolerance are still unaddressed — there's now enough real data to start on them |
